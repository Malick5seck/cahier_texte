...
public class SeanceDAO {
    ...
    public static List<Seance> getSeancesByEnseignant(int idEnseignant) {
        ...
        String query = "SELECT s.dateseance AS date, s.heure_debut, s.contenu, s.statut, " +
        ...
        try (Connection conn = DBconnect.getconnection();
            ...
            while (results.next()) {
                ...
                Seance s = new Seance(nomCours, date, heureDebut, contenu, statut, commentaire);
                seances.add(s);
            }
...
// Classe SeanceDAO.java dans DBO
package DBO;
...
public class SeanceDAO {
    ...
    public static List<Seance> getSeancesNonValidees() {
        List<Seance> seances = new ArrayList<>();
        String sql = "SELECT s.id, s.dateseance, s.contenu, c.nom AS nomCours " +
        ...
        try (Connection conn = DBconnect.getconnection();
            ...
            while (rs.next()) {
                ...
                Seance s = new Seance(id, nomCours, date, contenu);
                seances.add(s);
            }
    ...
    public static void updateStatut(int idSeance, String statut, String commentaire) {
...
public class SeanceDAO {
    ...
    public static List<Seance> getSeancesNonValidees() {
        ...
        String query = """
            SELECT s.id, s.dateseance, s.heure_debut, s.contenu, c.nom AS nomCours
            FROM seance s
            JOIN cours c ON s.cours_id = c.id
            WHERE s.statut = 'en attente'
        ...
        try (Connection conn = DBconnect.getconnection();
            ...
            while (rs.next()) {
                ...
                Seance s = new Seance(id, nomCours, date, heureDebut, contenu, "en attente", null);
                seances.add(s);
            }
    ...
    public static List<Seance> getSeancesByEnseignant(int idEnseignant) {
        ...
        String query = """
            SELECT s.dateseance AS date, s.heure_debut, s.contenu, s.statut, 
                   s.commentaire_refus AS commentaire, c.nom AS nomCours
            FROM seance s
            JOIN cours c ON s.cours_id = c.id
            WHERE s.Enseignant_id = ?
